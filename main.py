...
boards = jira.boards().iterable
teamBoard = list(filter(lambda board: teamName in board.name, boards))[-1]
teamSprintList = list(filter(lambda sprint: (teamName in sprint.name) | (altTeamName in sprint.name), jira.sprints(teamBoard.id)))
teamSprintListSinceDate = list(filter(lambda sprint: datetime.datetime.strptime(jira.sprint_info(teamBoard.id, sprint.id)['startDate'], '%d.%m.%Y %H:%M') > datetime.datetime.strptime(startDate, '%d.%m.%Y %H:%M'), teamSprintList))
for sprint in teamSprintListSinceDate:
    issuesForSprint = jira.search_issues('Sprint = ' + str(sprint.id))
    finishedItems = list(filter(lambda issue: issue.fields.status.name in ('Abgenommen', 'Fertig'),issuesForSprint))
    finishedItemsPerSprint[sprint.name] = len(finishedItems)


for sprint in range(firstSprint, lastSprint + 1):
    finishedItems = []
    for issue in jira.search_issues('project = MIELCLD3 AND component = Professional AND Sprint = "Team Professional Sprint ' + str(sprint) + '"', expand='changelog'):
